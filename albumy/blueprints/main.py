...
from albumy.notifications import (
    push_comment_notification, push_collect_notification
)
...
def show_photo(photo_id):
    ...
    return render_template(
        "main/photo.html", photo=photo, comment_form=comment_form,
    ...
    if form.validate_on_submit():
        for name in form.tag.data.split():
            ...
            if tag not in photo.tags:
                ...
                db.session.commit()
        flash("Tag add.", "success")
    
    flash_errors(form)
    return redirect(url_for("main.show_photo", photo_id=photo_id))


@main_bp.route("/delte/tag/<int:photo_id>/<int:tag_id>", methods=["POST"])
@login_required
...
@main_bp.route("/photo/<int:photo_id>/colectors")
