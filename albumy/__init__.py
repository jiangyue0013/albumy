...
from albumy.extensions import (
    bootstrap, db, mail, 
    moment, login_manager, dropzone,
    csrf, avatars, whooshee
)
from albumy.settings import config
from albumy.models import (
    Role, User, Permission, 
    Photo, Tag, Comment, 
    Collect, Follow, Notification
)
...
def register_commands(app):
    ...
    def forge(user, follow, photo, tag, collect, comment):
        """Generate fake data."""
        from albumy.fakes import (
            fake_admin, fake_user, fake_photo, 
            fake_comment, fake_tag, fake_collect,
            fake_follow
        )

        db.drop_all()
        db.create_all()
        click.echo("Initialing the roles and permissions...")
        Role.init_role()
        click.echo("Generating the administrators...")
        fake_admin()
